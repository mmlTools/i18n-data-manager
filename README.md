...
- **Configure once** - point it at the folder where your `*.json`, `*.ini`, `*.ts` or **CodeIgniter 4** PHP language files live (per-workspace setting).
...
- **Nested keys supported** - dot-notation in the UI (`common.buttons.submit`), nested JSON / PHP arrays on disk, and flat dotted keys in INI / TypeScript files.
- **Four file formats supported** - JSON (`en.json`), OBS-style INI (`en-US.ini`), TypeScript string maps (`pt_BR.ts`) and CodeIgniter 4 PHP arrays (`app/Language/en/Messages.php`). The format is auto-detected from your folder layout.
...
4. Click **Choose Translations Folder** and pick the folder containing your `en.json`, `fr.json`, `en-US.ini`, `pt_BR.ts`, etc. — or your CodeIgniter 4 `app/Language/` folder.
...
The extension supports these layouts. Pick whichever matches your project — the
**format is auto-detected** from what's on disk.

### Flat (JSON / INI / TypeScript)

One translation file per language inside the configured folder. The filename
(without `.json`, `.ini` or `.ts`) is taken as the language code:
...
TypeScript files are supported as flat exported string maps:
...
When a TypeScript locale file is edited from the sidebar, it is rewritten as a
sorted `export const <locale>: Record<string, string> = { ... }` map.
...
| ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- | ------- |
| `LocaleSynci18n.translationsPath`       | Folder containing your `*.json`, `*.ini`, `*.ts` or CodeIgniter 4 PHP language files (relative or abs.).                    | `""`    |
...
- Only `.json`, `.ini`, flat TypeScript string maps and CodeIgniter 4 `.php` array files are supported (not `.yaml`, `.po`, `.properties`, etc.).
- Pluralization rules (CLDR plural categories) aren't handled, values are treated as plain strings.
- Comments in JSON files aren't preserved (standard `JSON.parse` / `JSON.stringify`); comments in INI, TypeScript and CI4 PHP files are not preserved when the file is rewritten.
